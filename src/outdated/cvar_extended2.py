...
def get_nodes(num_days:int, num_fields : int, cap : float, rain_probs : list[float]):
    ...
    # should it be this?
    end_cap = (1 - np.mean(rain_probs)) * cap
    days_required = int(num_fields // end_cap) + 1
...
def run_cvar(params : ModelParams, seed : int, window : int, verbal : bool = False):
    ...
    # ### delete me
    Probs = [0.707576,0.313921,0.850093,0.476502,0.608962,0.25268,0.942063,0.472552,0.184014,0.509276,]
    ripe_days = [4,7,7,3,10,9,0,5,8,6,]
    # ###
    
    end_cap = (1 - np.mean(Probs)) * params.cap
    Probs = Probs[:window]
    ...
    return selected, sum([n.prob * Beta[n].X for n in end_nodes]), CVar.X
...
def main():
    ...
    for seed in tqdm(range(num_samples)):
        params = ModelParams(num_fields, num_days, cap, 0.2, True, 0)
        for Lambda in tqdm(range(0, 101, 10)):
            params = ModelParams(num_fields, num_days, cap, 0.2, False, Lambda / 100)
            _, eV, CVAR = run_cvar(params, seed, window, False)
            # print(current_lambda)
            EVs[Lambda].append(eV)
            CVARs[Lambda].append(CVAR)
            
    print('[' + ','.join([str(np.mean(EVs[Lambda])) for Lambda in range(0, 101, 10)]) + ']')
    print('[' + ','.join([str(np.mean(CVARs[Lambda])) for Lambda in range(0, 101, 10)]) + ']')
...
seed = 23
run_cvar(params, seed, params.num_days, verbal = True)
...
# t1 = time.time()
# run_cvar2(params, seed, params.num_days, verbal = True)
# t2 = time.time()
# print(t2 - t1)